...
if uploaded_file is not None:
    ...
    numbers_pattern = Pattern(name="numbers_pattern",
                              regex="[A-Z]{5}[0-9]{4}[A-Z]{1}", score=0.5)

    # Define the recognizer with one or more patterns
    number_recognizer = PatternRecognizer(
        supported_entity="PAN_NUMBER", patterns=[numbers_pattern])
    ...
    sensitive_attributes = ['User Id', 'Name',
                            'Twitter Username', 'User Bio', 'Profile URL']

    # Determine the K value
    K = 5

    # Identify the quasi-identifiers
    quasi_identifiers = ['Tweet Location', 'User Followers', 'User Following',
                         'User Account Creation Date', 'Tweet Posted Time (UTC)']
    ...
    anonymized_data = pd.concat(generalized_data)[
        ['Tweet Location', 'User Followers', 'User Following', 'User Account Creation Date', 'Tweet Content', 'Tweet Posted Time (UTC)']]
    ...
    for name, group in anonymized_data.groupby(quasi_identifiers):
        if len(group) < K:
            ...
            merged_group = pd.concat([group, anonymized_data.loc[anonymized_data.groupby(
                quasi_identifiers).groups[name]].iloc[0:K-len(group)]])
